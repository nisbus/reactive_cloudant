...
namespace ReactiveCloudant
{
    public class CloudantSession
    {
        ...
        internal void Poll<T>(string since, bool include_docs, IScheduler convererScheduler, string url, Subject<Poll<T>> outer)
        {
            ...
            {
                HttpClientExtensions.DownloadAndConvertChangesAsObservable<T>(new Uri(url), Username, Password, converterScheduler: convererScheduler, includes_docs: include_docs)
                    .Subscribe((data) =>
                    {
                        last = data.Since;
                        outer.OnNext(data);
        ...
        internal async Task<string> SetID(string json, string id)
        {
            if (string.IsNullOrWhiteSpace(json))
