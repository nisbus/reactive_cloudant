                        ...
                        try
                        {
                            ...
                            if (attachments != null)
                            {
                                foreach (JProperty o in attachments.Value)
                                {
                                    ...
                                    int l = 0;
                                    if (int.TryParse(o.Value["length"].ToString(), out l))
                                        value.Length = l;
...
/*
                ...
                try
                {
                    Task.Factory.StartNew(async () =>
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(username) && !string.IsNullOrWhiteSpace(password))
                                req.Headers["Authorization"] = "Basic " + Convert.ToBase64String(Portable.Text.ASCIIEncoding.ASCII.GetBytes(username + ":" + password));
        ...
        internal static void SetAuthenticationHeaders(this HttpClient client, string username, string password)
        {
            if (!string.IsNullOrWhiteSpace(username) && !string.IsNullOrWhiteSpace(password))
            {
                ...
                client.DefaultRequestHeaders.Add("Authorization", "Basic "+ Convert.ToBase64String(Portable.Text.Encoding.ASCII.GetBytes(authInfo)));
            }
        }
